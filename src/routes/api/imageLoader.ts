...
imageLoader.get('/', async (req, res): Promise<void> => {
  ...
  const fileNamesnoExtn = fileNames.map((file: string) => {
    return file.replace(/\.[^/.]+$/, '');
  });
  ...
  if (
    ...
    imageProcessor.imageProcessor(fileName, width, height).then(function () {
